...
pub fn create_log_file(file_name : Option<String>) -> Result<File, std::io::Error>{
    let file_name : String = file_name.unwrap_or("log.log".to_string());
    let file = OpenOptions::new().create(true).append(true).open(file_name.as_str())?;
    Ok(file)
}
...
pub fn get_subscriber(env : &str, name : &str, exporter_url : &str)-> impl Subscriber + Send+ Sync{
    ...
    let file = create_log_file(None).expect("Failed to create file name");
...
pub fn initialize_subscriber(subscriber: impl Subscriber + Sync + Send) {
    LogTracer::init().expect("Failed to initialize the log - tracer "); // Used to convert logs to traces
